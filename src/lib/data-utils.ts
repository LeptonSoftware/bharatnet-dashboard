...
function getWeeklyDelta(
  data: BlockData[],
  statusCheck: (item: BlockData) => boolean
): number {
  ...
  return data.filter((item) => {
    const date =
      parseDate(item.dtpSubmissionDate) || parseDate(item.dtpApprovalDate);
...
export function calculateSummaryStats(data: BlockData[]): SummaryStats {
  ...
  data.forEach((item) => {
    const status = String(item.dtpStatus || "").toLowerCase();
  ...
  stats.weeklySubmitted = getWeeklyDelta(
    data,
    (item) =>
      item.dtpStatus?.toLowerCase().includes("submitted") ||
      item.dtpStatus?.toLowerCase().includes("approved")
  );
...
export function getDistrictSummaries(data: BlockData[]): DistrictSummary[] {
  ...
  return Array.from(districts.values()).sort((a, b) => b.total - a.total);
}
...
export function calculateProjectProgress(data: BlockData[]): ProjectProgress {
  ...
  const daysSinceStart = Math.max(
    1,
    (now.getTime() - projectStartDate.getTime()) / (1000 * 60 * 60 * 24)
  );
