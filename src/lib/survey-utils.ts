import { SummaryStats, SurveyData, SurveyDistrictSummary } from "@/types"

export function calculateSurveySummaryStats(
  data: SurveyData[],
  ...
  data.forEach((item) => {
    ...
    if (type === "feasibility") {
      if (circle === "punjab") {
        stats.totalPlannedKm += (item.newOfc || 0) + (item.existingOfc || 0)
      } else {
...
export function getSurveyDistrictSummaries(
  data: SurveyData[],
  type: "feasibility" | "hoto",
  circle?: string,
): SurveyDistrictSummary[] {
  ...
  data.forEach((item) => {
    ...
    if (type === "feasibility") {
      if (circle === "punjab") {
        district.totalKm += (item.newOfc || 0) + (item.existingOfc || 0)
      } else {
  ...
  return Array.from(districts.values()).sort(
    (a, b) => b.completed - a.completed,
  )
}

export function getSurveyProgressData(
  data: SurveyData[],
  type: "feasibility" | "hoto",
) {
  ...
  const dates = data
    .filter((item) => {
      const endDate =
        type === "feasibility" ? item.feasibilityEndDate : item.hotoEndDate
      const status =
        type === "feasibility" ? item.feasibilityStatus : item.hotoSurveyStatus
  ...
  estimatedCompletionDate.setMonth(
    estimatedCompletionDate.getMonth() + monthsToComplete,
  )
...
export function getSurveyStatusDistribution(
  data: SurveyData[],
  type: "feasibility" | "hoto",
) {
  const distribution = {
