...
export function SurveyOverviewChart({
  ...
  const blockData = blockDistribution.labels.map((label, index) => ({
    name: label === 'completed' ? 'Completed' :
          label === 'ongoing' ? 'In Progress' : 'Pending',
    ...
    <Card className="col-span-full xl:col-span-6">
      ...
      <CardContent>
        <Tabs defaultValue="kilometers">
          ...
          <TabsContent value="kilometers" className="h-[300px]">
          ...
          <TabsContent value="blocks" className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={blockData}
