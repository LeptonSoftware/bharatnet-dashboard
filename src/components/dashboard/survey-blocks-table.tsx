...
export function SurveyBlocksTable({ data, type }: SurveyBlocksTableProps) {
  ...
  const columns: ColumnDef<SurveyData>[] = [
    ...
    {
      accessorKey: type === 'feasibility' ? "feasibilityStatus" : "hotoSurveyStatus",
      header: "Survey Status",
      cell: ({ row }) => {
        const status = row.getValue(type === 'feasibility' ? "feasibilityStatus" : "hotoSurveyStatus") as string;
        ...
        if (status.toLowerCase().includes('wip') || status.toLowerCase().includes('progress')) {
    ...
    {
      accessorKey: type === 'feasibility' ? "feasibilityOfcDone" : "hotoOfcDone",
      header: "Completed (km)",
      cell: ({ row }) => {
        const value = row.getValue(type === 'feasibility' ? "feasibilityOfcDone" : "hotoOfcDone") as number ?? 0;
        return <div className="text-right">{Number(value)?.toFixed(1) || '0.0'}</div>;
      },
    },
    {
      accessorKey: type === 'feasibility' ? "feasibilityOfcPending" : "hotoOfcPending",
      header: "Pending (km)",
      cell: ({ row }) => {
        const value = row.getValue(type === 'feasibility' ? "feasibilityOfcPending" : "hotoOfcPending") as number;
        return <div className="text-right">{Number(value)?.toFixed(1) || '0.0'}</div>;
      },
    },
  ];
                  ...
                  <tr key={rowIndex} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                    {columns.map((column, colIndex) => (
                      <td key={colIndex} className="p-4 align-middle">
                        {column.cell?.({ row: { getValue: () => row[column.accessorKey as keyof SurveyData] } })}
                      </td>
                    ))}
                  </tr>
                ))}
